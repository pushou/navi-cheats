---
nom: nushell.cheat
description: Cheatsheet navi de commandes Nushell — sys, CSV, JSON, Docker, osquery, ICS, journalctl
type: source
...
# Source : nushell.cheat

> Cheatsheet navi original de pushou, catégorisé `category:shell`.

## Description

Fichier de cheatsheet navi très riche couvrant : les built-ins système (`sys`, `cal`), le traitement de CSV/JSON/XLSX/ODS, l'intégration Docker, osquery, les adresses IP, le monitoring système, journalctl avancé, MTR, et les fichiers ICS (calendrier).

## Métadonnées

| Champ | Valeur |
|-------|--------|
| Fichier | `sources/nushell.cheat` |
| Auteur | pushou |
| Tags | `nushell, shell, terminal, category:shell` |
| Sections | ~25 |
| Type | Cheatsheet navi |

## Pages wiki alimentées

- [[entites/nushell.md]] — Page d'entité Nushell

## Contenu brut

```
% nushell,nushell,shell,terminal,category:shell,pushou
